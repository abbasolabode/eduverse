...
export function useApplicationForm() {
	...
	const { mutate: applyForm, isLoading: isApplyingForm } = useMutation({
		mutationFn: ({ data, confirmed }) => {
			//Early return
			if (!data) return;
			...
			formData.append("confirmation", confirmed);
			
			return getApplicationForm(formData); // Function to call the API for creating new data
		},
		onSuccess: () => {
	...
	return { applyForm, isApplyingForm };
}
